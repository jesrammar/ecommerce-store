...
def _resolver_variante(producto_id: int, meta: dict):
    ...
    if talla: qs = qs.filter(talla=talla)
...
def _pedidoitem_kwargs_base(pedido: Pedido, producto: Producto,
                            precio_unit: Decimal, qty: int,
                            subtotal: Decimal, meta: dict) -> dict:
    data = {
        "pedido": pedido,
...
def crear_pedido_desde_carrito(request, datos: dict) -> Pedido:
    ...
    pedido = Pedido.objects.create(
        ...
        telefono=datos.get("telefono", ""), direccion=datos["direccion"],
        ciudad=datos["ciudad"], cp=datos["cp"], total=Decimal("0.00"),
    ...
    for item in cart:
        producto = item.get("product") or Producto.objects.get(pk=item.get("product_id"))
        ...
        if variante:
            ...
            if qty > disponible:
                nombre_var = f"{getattr(variante,'talla','-')}/{getattr(variante,'color','-')}"
                raise ValueError(f"Sin stock suficiente para {producto.nombre} ({nombre_var}). Disponible: {disponible}.")
        else:
            disponible = _to_int_safe(getattr(producto, "stock", 0), 0)
            if qty > disponible:
                raise ValueError(f"Sin stock suficiente para {producto.nombre}. Disponible: {disponible}.")

        price_unit = _precio_unitario(item, base_precio=producto.precio, variante=variante)
        ...
        kwargs = _pedidoitem_kwargs_base(pedido, producto, price_unit, qty, line_total, meta)
...
def crear_pedido_tarjeta_pre(request, datos: dict):
    ...
    pedido = Pedido.objects.create(
        ...
        telefono=datos.get("telefono", ""), direccion=datos["direccion"],
        ciudad=datos["ciudad"], cp=datos["cp"], total=Decimal("0.00"),
        pago_metodo="tarjeta", pago_estado="iniciado",
    )
    ...
    for it in pedido.items.select_related("producto"):
        ...
        if Variante and var_id:
            try:
                ...
                variante.stock = max(0, _to_int_safe(variante.stock) - _to_int_safe(it.cantidad))
                variante.save(update_fields=["stock"])
            except Exception:
                pass
        producto.stock = max(0, _to_int_safe(producto.stock) - _to_int_safe(it.cantidad))
        producto.save(update_fields=["stock"])
...
def _enviar_email_confirmacion(pedido: Pedido) -> None:
    ...
    cuerpo = (
        ...
        f"Seguimiento: {getattr(settings, 'SITE_URL', 'http://127.0.0.1:8000')}/pedidos/seguimiento/{pedido.tracking_token}/\n"
    )
    try:
        send_mail(
            asunto, cuerpo,
